...
if [[ -z "$(ls ${assemblies}/*_nextpolished.fasta 2>/dev/null | grep fasta)" ]]; then
  echo "Correct path to the short-read polished assemblies not entered (*_nextpolished.fasta)"
  exit
fi
...
# Check if the reference variable is in the list of valid names
...
mkdir -p ${results}/mitofinder_polished ${results}/mitofinder_results

for x in ${assemblies}/*_nextpolished.fasta ; do 
    sample=${x##*/}
    name=${sample%_nextpolished.fasta}
    if [ -d "${results}/mitofinder_results/${name}_mitofinder_polished_Final_Results" ]; then
      echo "MitoFinder has already annotated polished assemblies for ${name}"
    elif [ -f "logs/${name}_mitofinder_polished_hydra.log" ]; then
      if [ -d ${results}/mitofinder_polished/${name}_mitofinder_polished ]; then
        if [ -f ${results}/mitofinder_results/${name}_mitofinder_polished_hydra.log ]; then
          rm -r ${results}/mitofinder_polished/${name}_mitofinder_polished ${results}/mitofinder_results/${name}_mitofinder_polished_hydra.log \
          ${results}/mitofinder_polished/${name}_mitofinder_polished_MitoFinder.log logs/${name}_mitofinder_polished_hydra.log
          qsub -o ${results}/../../jobs/logs/${name}_mitofinder_polished_hydra.log \
          -wd ${results}/mitofinder_polished \
          -N ${name}_mitofinder_polished \
          mitofinder_annotate_polished_loop.job ${assemblies} ${name} ${mito_code} ${ref_database} ${sample} ${results} ${path_to_ref}
        else
          rm -r ${results}/mitofinder_polished/${name}_mitofinder_polished ${results}/mitofinder_polished/${name}_mitofinder_polished_MitoFinder.log \
          logs/${name}_mitofinder_polished_hydra.log
          qsub -o ${results}/../../jobs/logs/${name}_mitofinder_polished_hydra.log \
          -wd ${results}/mitofinder_polished \
          -N ${name}_mitofinder_polished \
          mitofinder_annotate_polished_loop.job ${assemblies} ${name} ${mito_code} ${ref_database} ${sample} ${results} ${path_to_ref}
        ...
        rm logs/${name}_mitofinder_polished_hydra.log
        qsub -o ${results}/../../jobs/logs/${name}_mitofinder_polished_hydra.log \
        -wd ${results}/mitofinder_polished \
        -N ${name}_mitofinder_polished \
        mitofinder_annotate_polished_loop.job ${assemblies} ${name} ${mito_code} ${ref_database} ${sample} ${results} ${path_to_ref}
...
# sequences as an input, but instead uses the illumina short-read polished,
# medaka-corrected flye assemblies
