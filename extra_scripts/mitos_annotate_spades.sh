...
for x in ${contigs}/*.fasta; do
  ...
  mkdir -p ${results}/mitos_spades/${name}_mitos_spades \
  ${results}/mitos_results/${name}_mitos_spades
  
  qsub -o logs/${name}_mitos_spades.log \
  -N ${name}_mitos_spades \
  mitos_annotate_spades_loop.job ${contigs} ${results} ${sample} ${name} ${taxa} ${shortname}

  sleep 0.1
done

# This script uses the contigs from a spades output contig as a template
# for annotation, and submits a MITOS job for each contig in the source
# directory (typically results/spades_contigs/).


# This MITOS shell requires two elements after calling the script
# 1: path to the directory containing the spades contigs
# 2: the genetic code
